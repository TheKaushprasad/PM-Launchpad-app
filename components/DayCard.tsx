...
import { ArrowRight, Clock, ShieldCheck, CheckCircle2, Bookmark, FileEdit, Lock } from 'lucide-react';
...
export const DayCard: React.FC<DayCardProps> = ({ lesson, index, isCurrentLesson = false, onRequireLogin }) => {
  ...
  // Card container styling based on state (locked: subtle amber/zinc border, completed: green border, in-progress/current: subtle blue accent, not-started: neutral border)
  const cardStateClasses = isLocked
    ? 'bg-white/95 border-zinc-200/80 hover:border-indigo-300 hover:shadow-md'
    : isCompleted
    ...
    <>
      ...
      >
        ...
        <div className="mt-auto pt-4 border-t border-zinc-100 flex items-center justify-between gap-3">
          ...
          {/* Action Link / Status Icon */}
          {isLocked ? (
            <div 
              className="flex items-center justify-center text-zinc-400 group-hover:text-zinc-700 transition-colors"
