...
export const ToolsHub: React.FC = () => {
  ...
  const handleAction = (tool: { action: string; cta: string }) => {
    ...
    if (!user) {
      setPendingRedirect(tool.action);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full items-stretch">
          {tools.map((tool) => (
            <div 
              ...
              className="group bg-white rounded-2xl sm:rounded-3xl border border-zinc-200/90 p-6 sm:p-7 md:p-8 flex flex-col justify-between shadow-sm hover:shadow-md hover:border-zinc-300/90 transition-all duration-200 cursor-pointer"
            >
