        ...
        <nav className="flex-1 overflow-y-auto px-4 py-4 space-y-1 no-scrollbar">
          {NAV_ITEMS.map((item, index) => {
            if (item.type === 'divider') {
               ...
               ) : <div key={index} className="h-px bg-zinc-100 my-6 mx-2" />
            }
            ...
            return (
              ...
              >
                ...
                {!collapsed && <span className="flex-1 relative z-10 tracking-tight">{item.label}</span>}
                {!collapsed && isActive && <ChevronRight className="w-4 h-4 text-[#79BAEC] relative z-10" />}
              </Link>
            );
          })}
        </nav>
